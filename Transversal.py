...
while True:
    ...
    if json_status == 0:
        ...
        distance_km = round(json_data["route"]["distance"] * 1.61, 1)
        print("Kilometers: " + str(distance_km))
        distance_miles = round(json_data["route"]["distance"], 1)
        print("Miles: " + str(distance_miles))

        if "fuelUsed" in json_data["route"]:
            fuel_used_gal = json_data["route"]["fuelUsed"]
            fuel_used_ltr = round(fuel_used_gal * 3.78, 1)
        ...
        for each in json_data["route"]["legs"][0]["maneuvers"]:
            distance_km = round(each["distance"] * 1.61, 1)
    ...
    stop = input("Press 's' to stop or any other key to continue: ")
    if stop == "s":
        break
